...
// Sayfa için Props türünü tanımla
interface Props {
  searchParams: {
    date?: string
    price?: string
...
export default async function Page({ searchParams }: Props) {
  ...
  // Sıralama ve filtreleme parametrelerini oluştur
  ...
  const filter = `*[_type == "product" && _id != null${colorFilter}${categoryFilter}${sizeFilter}${searchFilter}]`
        ...
        <main className="mx-auto max-w-6xl px-6">
          <div className="flex items-center justify-between border-b border-gray-200 pb-4 pt-24 dark:border-gray-800">
            <h1 className="text-xl font-bold tracking-tight sm:text-2xl">
              {products.length} Ürün {products.length === 1 ? "" : ""}
            </h1>
            {/* Ürün Sıralama */}
            <ProductSort />
          </div>
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className={cn("grid grid-cols-1 gap-x-8 gap-y-10", products.length > 0 ?
              'lg:grid-cols-4' : 'lg:grid-cols-4[1fr_3fr]')}>
              <div className="hidden lg:block">{/* Ürün filtreleri */}
                <ProductFilters />
              </div>
              {/* Ürün grid */}
